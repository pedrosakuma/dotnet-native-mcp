...
blob we do not parse into managed metadata. Decoding them requires the
`NativeFormat` hashtable reader plus `Internal.TypeSystem` type resolution —
partly redundant with `dotnet-assembly-mcp` (which owns managed metadata):

`InstanceMethodEntryPoints`,
`InliningInfo*`, `ProfileDataInfo`, `PgoInstrumentationData`, `AttributePresence`,
`CrossModuleInlineInfo`, the remaining `*Map` sections,
`DelayLoadMethodCallThunks`, `ExceptionInfo`, and `DebugInfo`. The
`ManifestMetadata` (112) blob is *located* (offset/size/version/streams) for
handoff but its managed metadata is decoded by `dotnet-assembly-mcp`.
